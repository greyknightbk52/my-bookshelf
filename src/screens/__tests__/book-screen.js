...
test("can remove a list item for the book", async () => {
  ...
  act(() => userEvent.click(removeFromListButton));
...
test("can mark a list item as read", async () => {
  ...
  const listItem = await listItemsDB.create(
    buildListItem({ owner: user, bookId: book.id, finishDate: null })
  );
  await renderBookScreen({ user, book, listItem });
  const markAsReadButton = screen.getByRole("button", {
    name: /mark as read/i,
  });
  act(() => userEvent.click(markAsReadButton));
...
describe("console errors", () => {
  ...
  test("shows an error message when the boook fails to load", async () => {
  ...
  test("note update failures are displayed", async () => {
    jest.useFakeTimers();
    await renderBookScreen();
    const newNotes = faker.lorem.word();
    const notesTextArea = screen.getByRole("textbox", { name: /notes/i });

    const testErrorMessage = "__test_error_message__";
    server.use(
      rest.put(`${apiUrl}/list-items/:listItemId`, (req, res, ctx) => {
        return res(
          ctx.status(400),
          ctx.json({ message: testErrorMessage, status: 400 })
        );
      })
    );
    act(() => {
